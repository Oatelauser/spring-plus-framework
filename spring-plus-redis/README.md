...
Redis 能力域独立模块（ADR 0003，自 `spring-plus-boot` 拆出）：`StringRedisTemplate` 增强工具，Lua 脚本保证原子性。
...
<dependency>
    ...
    <artifactId>spring-plus-redis</artifactId>
...
- 从 `spring-plus-boot` 迁移而来：包名 `springplus.boot.redis` → `springplus.redis`，坐标由 boot 替换为本模块
