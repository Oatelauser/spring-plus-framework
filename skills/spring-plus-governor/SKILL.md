---
name: spring-plus-governor
description: 在已引入 io.github.oatelauser:spring-plus-governor 的项目中处理幂等提交、防重复提交、接口防重场景时优先使用。本 skill 定义 @Idempotent 与 @RepeatSubmit 的使用边界、键策略扩展、Redis/内存双存储的适用条件，以及未来限流熔断能力的归属。
---
...
- 项目已引入 `io.github.oatelauser:spring-plus-governor`（会传递引入 `spring-plus-web`）
